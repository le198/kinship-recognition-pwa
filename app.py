...
# Patch functions (dùng để sửa Lambda layer)
...
def load_kinship_model():
    path = "facenet_vgg.keras"
    if not os.path.exists(path):
        st.error(f"Model file `{path}` NOT FOUND! Please upload it to the repo root.")
        return None
    try:
        with st.spinner("Loading AI model (this may take 30-60s on first run)..."):
            model = load_model(
                path,
                custom_objects={
        ...
        # PATCH LAMBDA LAYERS (dựa trên tên layer)
        st.info("Patching Lambda layers...")
...
st.markdown("### Chụp từ webcam hoặc upload 2 ảnh → kiểm tra quan hệ họ hàng")
...
if (uploaded1 or img1_input) and (uploaded2 or img2_input) and model and face_net:
    ...
    if face1 is None or face2 is None:
        st.error("No face detected. Please take a clear photo.")
        st.stop()

    st.subheader("Original + BBox")
    c1, c2 = st.columns(2)
    with c1: st.image(cv2.cvtColor(img1_box, cv2.COLOR_BGR2RGB), caption=f"Photo 1 (conf: {conf1:.2f})", use_container_width=True)
    with c2: st.image(cv2.cvtColor(img2_box, cv2.COLOR_BGR2RGB), caption=f"Photo 2 (conf: {conf2:.2f})", use_container_width=True)

    st.subheader("Cropped Face (224×224)")
    c3, c4 = st.columns(2)
    ...
    try:
        ...
        if score > threshold:
            st.success(f"**FAMILY RELATIONSHIP** (score > {threshold})")
        else:
            st.warning(f"**NO RELATIONSHIP** (score ≤ {threshold})")

        with st.expander("Technical Details"):
            st.write("- **FaceNet**: Geometric features (160×160)\n- **VGG-Face**: Texture & shape (224×224)\n- Final score: Cosine similarity")
    ...
    st.info("Please select **2 photos** (upload or webcam).")
    if not model:
        st.error("Model not loaded. Upload `facenet_vgg.keras` to repo root.")
    if not face_net:
        st.error("Face detector failed.")

st.caption("**PWA**: Open in Chrome Android → Add to Home screen → Install as app!")